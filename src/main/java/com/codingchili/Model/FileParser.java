 ...
 */
public class FileParser implements Publisher<JsonObject> {
    ...
    /**
     * Parses the contents of an excel into JSON.
     *
     * @param file   file of an excel file.
     * @param offset row number containing column titles.
     */
    public FileParser(File file, int offset, String fileName) throws ParserException, FileNotFoundException {
    ...
    public void subscribe(Subscriber<? super JsonObject> subscriber) {
        subscriber.onSubscribe(new Subscription() {
            ...
            public void request(long count) {
                readRows(index, count, subscriber::onNext, false);
                index += count;
     ...
     * for the result. This should be called before{@link #parseRowRange(int, int, Consumer)} to make
     * sure any imports does not fail halfway through.
     */
    public void assertFileParsable() {
        logger.parsingFile(fileName, offset);

        // parse all rows.
        readRows(offset, rows, (json) -> {
    ...
    public void parseRowRange(int begin, int count, Consumer<JsonObject> consumer) {
        readRows(begin, begin + count, consumer, false);
    }
     ...
     */
    private void readRows(int start, long count, Consumer<JsonObject> consumer, boolean dryRun) {
